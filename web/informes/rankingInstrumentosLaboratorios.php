...
function Footer()
{
    //ACA PONEMOS EL TOTAL GENERAL
        ...
        and ins.lab_cod=lab.lab_cod
        and lab.lab_cod=$codlaboratorio
        and ing.fecha_recepcion >= '$desde'
        and ing.fecha_recepcion <= '$hasta'");
        
        $cantidad=pg_result($consulta,0,'cantidad');
        $this->SetFont('Arial','',16);
        $this->text(10,275,'TOTAL GENERAL');
        $this->text(70,275,$cantidad);
...
function Header()
{
   ...
   if  (empty($_POST['txtLaboratorio'])){$codlaboratorio='';}else{ $codlaboratorio = $_POST['txtLaboratorio'];}
    ...
    $this->SetTitle('Clientes-Ranking Instrumentos');
    $this->text(55,50,'CONTROL DE CALIBRACION INSTRUMENTOS');
    $this->text(65,60,'Ranking de Instrumentos por Laboratorios');//Titulo
    //$this->text(45,65,$cliente);
    $this->text(10,75,'LABORATORIO:');
    $this->text(45,75,$laboratorio);
...
$pdf->Cell(100,10,'Instrumento',1,1,'C',50);
...
{   
    ...
    $pdf->Cell(100,5,$instrumento,1,1,'L',$fill);
    $fill=!$fill;
    $i++;
}
